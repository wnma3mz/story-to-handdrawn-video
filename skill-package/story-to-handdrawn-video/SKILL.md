...
12. Build the lossless narration master, no-cover voiced master, and audible-cover release with `scripts/build_story_audio.py`. For long unattended work, submit `scripts/async_story_job.py submit`; report its job ID immediately, inspect state/logs without blocking, and use `resume` after a failure so completed hash-validated stages are reused.
