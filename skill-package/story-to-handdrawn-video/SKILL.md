...
Read `references/code-plate-motifs.md` when Image2 is unavailable or the story is compatible with procedural SVG illustrations.
...
- `raster` (default): PNG images from Image2 generation or upload. Requires image generation pipeline.
- `svg`: static SVG file under `public/`. Use when hand-crafted vector illustrations are available.
- `code`: procedural SVG drawn by the renderer from a named motif (see `references/code-plate-motifs.md`). **No image generation required** — ideal for LLM agents without Image2 access.
